...
self.addEventListener('message', async (event) => {
  ...
  try {
    if (type === 'init') {
      ...
      const items = data; // массив растворов с полем description_en
      const embeddings: Record<number, number[]> = {};

      const descriptions = items.map((item: any) => item.description_en);
      ...
      const embeddingSize = 768; // для SigLIP base
      ...
      console.log('Text embeddings computed for', Object.keys(embeddings).length, 'items');
    }
