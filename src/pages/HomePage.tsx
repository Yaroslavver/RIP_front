import { useEffect, useRef, useState } from 'react';
import { Link } from 'react-router-dom';
import Header from '../components/Header';
import { getElectrolytes } from '../api/servicesApi';
import { GUEST_ONLY } from '../config/runtime';
import { useElectrolyteSearch, type ProcessedElectrolyte } from '../hooks/useElectrolyteSearch';
import { addElectrolyteToDraft, loadCartInfo } from '../store/requestsSlice';
import { loadServicesError, loadServicesStart, loadServicesSuccess, setServicesSearch } from '../store/servicesSlice';
import { useAppDispatch, useAppSelector } from '../store/hooks';
import { DEFAULT_IMAGE, getAssetUrl, useDefaultImageOnError } from '../utils/assets';

const HomePage = () => {
  const dispatch = useAppDispatch();
  const { items, search, loading } = useAppSelector((state) => state.services);
  const { user } = useAppSelector((state) => state.auth);
  const { cart, loading: requestLoading } = useAppSelector((state) => state.requests);
  const [searchInput, setSearchInput] = useState(search);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const {
    items: searchResults,
    ready: clipReady,
    progress: clipProgress,
    imageEmbedding,
    searchByImage,
    resetSearch,
  } = useElectrolyteSearch(items);

  const loadServices = async (query: string) => {
    dispatch(loadServicesStart());
    try {
      const electrolytes = await getElectrolytes(query);
      dispatch(loadServicesSuccess(electrolytes));
    } catch {
      dispatch(loadServicesError('Не удалось загрузить растворы'));
    }
  };

  useEffect(() => {
    loadServices(search);
    if (user) dispatch(loadCartInfo());
  }, [dispatch, user]);

  const displayItems = imageEmbedding
    ? searchResults.filter((item) => item.isVisible)
    : items.map((item) => ({ ...item, score: 0, isVisible: true }) as ProcessedElectrolyte);

  const handleSubmit = (event: React.FormEvent) => {
    event.preventDefault();
    dispatch(setServicesSearch(searchInput));
    loadServices(searchInput);
  };

  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    setSelectedImage(URL.createObjectURL(file));
    searchByImage(file);
  };

  const handleClearImage = () => {
    setSelectedImage(null);
    resetSearch();
    if (fileInputRef.current) fileInputRef.current.value = '';
  };

  const handleAdd = (electrolyteId: number) => {
    if (!user) return;
    dispatch(addElectrolyteToDraft({ electrolyte_id: electrolyteId }));
  };

  return (
    <>
      <Header />
      <main>
        <div className="search-cart">
          <form onSubmit={handleSubmit} className="search-form">
            <input
              type="text"
              name="search"
              placeholder="Поиск по названию..."
              value={searchInput}
              onChange={(event) => setSearchInput(event.target.value)}
              className="search-input"
            />
            <button type="submit" className="search-btn" disabled={loading}>Найти</button>
          </form>
          {!GUEST_ONLY && user && cart.count > 0 ? (
            <Link to={`/concentration/${cart.draft_id}`} className="cart-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                <path fillRule="evenodd" d="M7.21.8C7.69.295 8 0 8 0c.109.363.234.708.371 1.038.812 1.946 2.073 3.35 3.197 4.6C12.878 7.096 14 8.345 14 10a6 6 0 0 1-12 0C2 6.668 5.58 2.517 7.21.8zm.413 1.021A31.25 31.25 0 0 0 5.794 3.99c-.726.95-1.436 2.008-1.96 3.07C3.304 8.133 3 9.138 3 10c0 0 2.5 1.5 5 .5s5-.5 5-.5c0-1.201-.796-2.157-2.181-3.7l-.03-.032C9.75 5.11 8.5 3.72 7.623 1.82z" />
                <path fillRule="evenodd" d="M4.553 7.776c.82-1.641 1.717-2.753 2.093-3.13l.708.708c-.29.29-1.128 1.311-1.907 2.87l-.894-.448z" />
              </svg>
              <span className="cart-count">{cart.count}</span>
            </Link>
          ) : (
            <span className="cart-icon inactive">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                <path fillRule="evenodd" d="M7.21.8C7.69.295 8 0 8 0c.109.363.234.708.371 1.038.812 1.946 2.073 3.35 3.197 4.6C12.878 7.096 14 8.345 14 10a6 6 0 0 1-12 0C2 6.668 5.58 2.517 7.21.8z" />
                <path fillRule="evenodd" d="M4.553 7.776c.82-1.641 1.717-2.753 2.093-3.13l.708.708c-.29.29-1.128 1.311-1.907 2.87l-.894-.448z" />
              </svg>
            </span>
          )}
        </div>

        <div className="clip-search">
          <input type="file" accept="image/*" ref={fileInputRef} hidden onChange={handleImageUpload} />
          <button className="btn" onClick={() => fileInputRef.current?.click()} disabled={!clipReady || loading}>
            {clipReady ? 'Поиск по фото' : `Модель ${Math.round(clipProgress)}%`}
          </button>
          {selectedImage && <button className="btn secondary" onClick={handleClearImage}>Сбросить фото</button>}
          {selectedImage && <img src={selectedImage} alt="preview" className="preview-image" />}
        </div>

        <div className="container">
          <div className="cards-grid">
            {displayItems.map((electrolyte) => (
              <article className="card" key={electrolyte.id}>
                <Link to={`/electrolyte/${electrolyte.id}`}>
                  <img src={getAssetUrl(electrolyte.image) || DEFAULT_IMAGE} alt={electrolyte.name} onError={useDefaultImageOnError} />
                </Link>
                <div className="card-body">
                  <h3><Link to={`/electrolyte/${electrolyte.id}`}>{electrolyte.name}</Link></h3>
                  <p>Концентрация: {electrolyte.concentration} М</p>
                  <p className="ions">Ионы: {electrolyte.ions}</p>
                  {electrolyte.score > 0 && <p className="small">Сходство: {Math.round(electrolyte.score * 100)}%</p>}
                  {!GUEST_ONLY && (
                    <button className="btn" type="button" onClick={() => handleAdd(electrolyte.id)} disabled={!user || requestLoading}>
                      Добавить
                    </button>
                  )}
                </div>
              </article>
            ))}
            {displayItems.length === 0 && <p>Ничего не найдено</p>}
          </div>
        </div>
      </main>
    </>
  );
};

export default HomePage;
