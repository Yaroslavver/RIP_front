...
import { loadElectrolyteServices, setServicesSearch } from '../store/servicesSlice';
...
const HomePage = () => {
  ...
  useEffect(() => {
    dispatch(loadElectrolyteServices());
  ...
  const handleAdd = (event: React.FormEvent<HTMLFormElement>, electrolyteId: number) => {
    event.preventDefault();
    if (!user) return;
    const volume = Number(new FormData(event.currentTarget).get('volume'));
    if (volume > 0) {
      dispatch(addElectrolyteToDraft({ electrolyte_id: electrolyteId, volume }));
    }
  };
            ...
            {displayItems.map((electrolyte) => (
              <article className="card" key={electrolyte.id}>
                ...
                <div className="card-body">
                  ...
                  <form className="add-form" onSubmit={(event) => handleAdd(event, electrolyte.id)}>
